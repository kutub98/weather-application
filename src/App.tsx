...
const App: React.FC = () => {
  ...
  const [currentLocationWeather, setCurrentLocationWeather] = useState<Weather | null>(null);
  ...
  async function fetchCurrentLocationWeather(lat: number, lon: number) {
    try {
      const res = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&appid=${API_KEY}`
      );
      if (!res.ok) throw new Error("Failed to fetch weather");
      const result: Weather = await res.json();
      setCurrentLocationWeather(result);
    } catch (err: unknown) {
